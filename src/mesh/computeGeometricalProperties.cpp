...
double compute_volume(Eigen::MatrixXd V, Eigen::MatrixXi F, Eigen::MatrixXd N) {
    ...
    if (V.rows() != 3 || F.rows() != 3 || N.rows() != 3)
        throw std::invalid_argument( "Error while calling \'compute_volume\', wrong input size" );
    ...
    for (int i=0; i<F.cols(); i++) {
        ...
        // get area of the face with the cross product between the two vectors of the triangle 
        double surface_area_temp = (v3-v1).cross(v2-v1).norm()/2;

        // get the signed height of the tetrahedron formed with (v1, v2, v3, (0,0,0))
...
double compute_surface_area(Eigen::MatrixXd V, Eigen::MatrixXi F) {
    ...
    for (int i=0; i<F.cols(); i++) {
        ...
        // using Heron's formula: https://en.wikipedia.org/wiki/Heron%27s_formula (same results as cross product)
        double a, b, c, s;
        a = (v3 - v1).norm();
        b = (v2 - v1).norm();
        c = (v2 - v3).norm();
