                                ...
                                const Eigen::MatrixXd & ED_target)
{
    ...
    polyscope::registerPointCloud("source", source_V.transpose());
    polyscope::getPointCloud("source")->setPointColor(glm::vec3{0.1, 0.1, 1});
                                ...
                                double offset)
{
    ...
    Eigen::VectorXd offset_vector = Eigen::VectorXd::Constant(target_V.rows(), offset);
    ...
    createCurveNetwork(source_V, target_V, correspondences_nodes, correspondences_edges);
                                ...
                                int correspondence_to_remove)
{
    ...
    Eigen::MatrixXd correspondences_nodes;
    Eigen::MatrixXi correspondences_edges;
    createCurveNetwork(source_V.row(correspondence_to_remove), target_V.row(correspondence_to_remove), correspondences_nodes, correspondences_edges);

    std::vector<std::array<double, 3>> edgeColor(correspondences_edges.rows());
    for (size_t i = 0; i < edgeColor.size(); i++)
        edgeColor[i] = {{0, 0, 1}};
