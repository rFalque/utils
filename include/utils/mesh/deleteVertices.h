...
inline void deleteVertices(Eigen::MatrixXd& V,
    ...
    std::sort (vertices_to_delete.begin(), vertices_to_delete.end(), std::greater<int>());
    ...
    for (int i=0; i<F.cols(); i++){
        ...
        for (int j=0; j<F.rows(); j++) {
            auto it = find (vertices_to_delete.begin(), vertices_to_delete.end(), F(j,i));
            if (it != vertices_to_delete.end())
                keep_face = false;
        }
