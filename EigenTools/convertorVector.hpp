...
template<typename T> Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> to_eigen(std::vector<std::vector<T>> input) {
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> output;

    for (auto array : input)
        if (array.size() != input[0].size())
            throw std::invalid_argument( "All the sub arrays do not have the same size" );
