...
GreedySearch::GreedySearch(Eigen::MatrixXd V, Eigen::MatrixXi F, std::vector<int> indexes_of_sub_V_in_V)
{
	...
	{
		adjacency_list_.at( F_(0, i) ).push_back(neighbor(F_(1, i), distance_between_points(V_.col(F_(0, i)), V_.col(F_(1, i)) ) ));
		adjacency_list_.at( F_(1, i) ).push_back(neighbor(F_(2, i), distance_between_points(V_.col(F_(1, i)), V_.col(F_(2, i)) ) ));
		adjacency_list_.at( F_(2, i) ).push_back(neighbor(F_(0, i), distance_between_points(V_.col(F_(2, i)), V_.col(F_(0, i)) ) ));
	}
...
void GreedySearch::remove_element(int element_to_remove){
	if (element_to_remove < indexes_of_sub_V_in_V_.size())
		indexes_of_sub_V_in_V_.erase(indexes_of_sub_V_in_V_.begin() + element_to_remove);
	else 
		throw std::invalid_argument( "the element to remove is larger than the size of sub_V" );
};
